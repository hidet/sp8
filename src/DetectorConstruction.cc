...
G4VPhysicalVolume* DetectorConstruction::ConstructVolumes()
{
  ...
  target_box = new G4Box("target_box",target_size[0]/2.,target_size[1]/2.,target_size[2]/2.);
  target_log  = new G4LogicalVolume(target_box,
                                    targetMat,
                                    "target_log",0,0,0);
  target_phys = new G4PVPlacement(rot1,
  ...
  //const G4int NCOLI=10;
  const G4int NCOLI=NbOfTES;
  // colimator1 (Si)
...
{
  if (material=="Pb")              targetMat = mat->Pb;
  else if (material=="NeGas")      targetMat = mat->NeGas;
